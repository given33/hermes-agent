...
KANBAN_GUIDANCE = (
    ...
    "When you need the user to pick a direction, format the reason as a "
    "question followed by numbered options — `选项：\\nA. ...\\nB. ...\\nC. ...` — "
    "the mobile client renders them as tappable choices (plus a custom "
    "answer). The user's answer comes back as an `UNBLOCK:` comment on your "
    "task; resume with that decision.\n"
    ...
    "3. Always wait for their results (`delegate_task` returns the summary "
    "synchronously) and fold every subagent's findings into YOUR handoff: "
    "`kanban_complete` / `kanban_request_review` `metadata` must include a "
    "`subagents` list: `[{\"goal\": \"...\", \"outcome\": \"completed\", "
    "\"summary\": \"...\"}]` (never secrets/PII). This is how the hosted "
    "dashboard renders your child-agent cards.\n"
    ...
    "subagent tree (ids + goals), and `subagent_send(subagent_id=..., "
    "text=...)` to push a direction adjustment into a running subagent — the "
    "instruction lands at its next iteration boundary without cutting its "
    "current tool call. Use it when a child's direction drifts, when new "
    "information changes the goal, or when a child should verify something "
    "it skipped.\n"
    ...
    "9. Name every subagent you spawn: pass `name` — a short Chinese job "
    ...
    "8. Team rules are injected into every subagent automatically (SOUL.md "
