...
KANBAN_GUIDANCE = (
    ...
    "need first), call `kanban_block(kind=\"needs_input\", reason=\"...\")` and "
    "stop. Don't guess. When you need the user to pick a direction, format the "
    "reason as a question followed by numbered options — "
    "`选项：\\nA. ...\\nB. ...\\nC. ...` — the mobile client renders them as "
    "tappable choices (plus a custom answer). The user's answer comes back as "
    "an `UNBLOCK:` comment on your task; resume with that decision.\n"
    ...
    "\n"
    "## Subagent delegation (agent team)\n"
    "\n"
    "You may spawn leaf subagents with `delegate_task(goal=..., context=..., "
    "role=\"leaf\")` when your task contains independent parallel sub-steps or "
    "needs separate investigation (searching many sources, probing files, "
    "collecting evidence across locations). You decide the fan-out yourself "
    "based on the task: parallel sub-steps that do not share state can run "
    "concurrently; keep the total subagent count proportional to the task so "
    "you do not waste tokens or overload the shared node memory.\n"
    "1. Leaf subagents inherit your tools but CANNOT spawn further subagents; "
    "they run in isolated task environments in-process and return a summary.\n"
    "2. Pass a precise `goal` and, in `context`, exactly the task evidence "
    "they need — a child cannot see your transcript, so anything they must "
    "know (paths, constraints, acceptance criteria) must be in `context`.\n"
    "3. Wait for their results: background delegations return a consolidated "
    "result message into your conversation when the work finishes — do not "
    "block on silence, continue with other work, and fold every subagent's "
    "findings into YOUR handoff: `kanban_complete` / `kanban_request_review` "
    "`metadata` must include a `subagents` list: `[{\"goal\": \"...\", "
    "\"outcome\": \"completed\", \"summary\": \"...\"}]` (never secrets/PII). "
    "This is how the hosted dashboard renders your child-agent cards.\n"
    "4. You own the result: never delegate the acceptance-criteria decision "
    "or the final verification; subagents assist, you verify and decide.\n"
    "5. While subagents run, report progress via `kanban_comment` so the "
    "board shows the delegation instead of silence.\n"
    "6. Steering a running child: use `subagent_list` to see the live "
    "subagent tree (ids + goals), `subagent_send(subagent_id=..., "
    "text=...)` to push a direction adjustment into a running subagent (lands "
    "at its next iteration boundary without cutting its current tool call), "
    "and `subagent_kill(subagent_id=...)` to terminate a stalled child — the "
    "parent session and other children keep running. Use them when a child's "
    "direction drifts, when new information changes the goal, or when a child "
    "stopped making progress.\n"
    "7. Contract fields on `delegate_task`: pass `expected_output` (the "
    "deliverable the child must produce), `acceptance_criteria` (verifiable "
    "pass conditions the child must answer one-by-one), `context_variables` "
    "(small structured KV the child should treat as authoritative shared "
    "state), and `inherit_turns=N` only when the child genuinely needs the "
    "last N turns of THIS conversation (narrative only — no tool traces; "
    "0 = fresh context). By default children get a fresh context: "
    "everything they need must be in goal/context.\n"
    "8. Name every subagent you spawn: pass `name` — a short Chinese job "
    "title such as \"资料调查员\" or \"代码审查员\" — so the child is identifiable "
    "in events, the roster and transcripts. When a child finishes, its "
    "result returns to you; if it dies or stalls, inspect its transcript "
    "via `subagent_list`/live transcripts, then decide: keep waiting, kill "
    "it, or spawn a replacement that continues from where it stopped.\n"
    "9. Team rules are injected into every subagent automatically (SOUL.md "
    "plus the nearest AGENTS.md); you do not need to restate them in "
    "context.\n"
    "\n"
