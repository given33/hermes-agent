...
KANBAN_GUIDANCE = (
    ...
    "during long operations and at least hourly; stale work is reclaimed.\n"
    "4. **Block only for external input.** Use "
    "`kanban_block(kind=\"needs_input\", reason=\"...\")`; do not guess. For "
    "a choice, include `选项：\\nA. ...\\nB. ...`; resume from the `UNBLOCK:` "
    "comment.\n"
    "5. **Finish through the task graph.** Put `summary` and secret-free "
    "`metadata` on the transition. Inspect child cards first. Complete a parent "
    "to release its pre-created review/QA/release children; otherwise use "
    "`kanban_request_review` for same-card review. Review rework uses "
    "`kanban_request_changes`, not a sticky block. Use `metadata=...` for "
    "secret-free evidence on the transition.\n"
    "   The task graph response lists child IDs; inspect those cards before "
    "acting on the parent. Complete the pre-created review, QA, or release "
    "child through the normal transition and call `kanban_complete` when its "
    "acceptance work is done. Never sticky-block that parent for "
    "`review-required`; use `kanban_request_changes` when a reviewer finds "
    "rework, with `metadata={...}` containing only secret-free evidence.\n"
    ...
    "\n"
    "Use `delegate_task(role=\"leaf\")` only for independent parallel research "
    "or implementation; keep fan-out proportional to the work.\n"
    "1. Name each child (for example 资料调查员), and pass a precise `goal`, "
    "all required evidence in `context`, plus `expected_output`, verifiable "
    "`acceptance_criteria`, and small authoritative `context_variables`.\n"
    "2. Children are isolated, cannot spawn children, and cannot see your "
    "transcript. They start fresh unless `inherit_turns=N` is genuinely needed; "
    "that inheritance contains narrative turns, not tool traces.\n"
    "3. Keep working while children run. Use `subagent_list` for live state and "
    "transcripts, `subagent_send` to steer at the next safe boundary, and "
    "`subagent_kill` for a stalled or drifting child. Other workers continue.\n"
    "4. You own verification and acceptance. Record progress with "
    "`kanban_comment`, then include each child's goal, outcome, and summary in "
    "the final `metadata.subagents` list (never secrets or PII).\n"
    "5. SOUL.md and the nearest AGENTS.md are injected automatically.\n"
    "\n"
