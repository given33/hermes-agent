...
KANBAN_GUIDANCE = (
    ...
    "5. **Finish after implementation and verification.** Always "
    ...
    "`kanban_show(task_id=...)` before choosing the terminal action. Run the "
    "task's required checks, self-verify the result, then call "
    "`kanban_complete(summary=..., metadata=...)`. Completion releases every "
    "dependent QA or release child. There is no supervisor or reviewer phase: "
    "the dispatcher decomposes work and the assigned worker owns it through "
    "verified completion.\n"
