...
from hermes_runtime.credential_persistence import sanitize_borrowed_credential_payload
...
_HERMES_CLI_VERSION = HERMES_VERSION
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    "nous": ProviderConfig(
        ...
        base_url_env_var="NOUS_INFERENCE_BASE_URL",
    ),
...
def get_anthropic_key() -> str:
    ...
    """
    from hermes_runtime.config import get_env_value_prefer_dotenv
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from agent.copilot_auth import resolve_copilot_token, get_copilot_api_token
...
def resolve_provider(
    ...
    *,
    ...
    try:
        ...
        _provider_config = PROVIDER_REGISTRY.get(_maybe) if _maybe else None
        if (
            _provider_config is not None
            and _provider_config.auth_type != "api_key"
...
def read_codex_tokens() -> Dict[str, Any]:
    """Public read-only access to the profile's validated Codex token state.

    Callers outside ``hermes_cli.auth`` must use this boundary instead of the
    lock-aware private helper.  Refresh and mutation remain owned by the auth
    service.
    """
...
def get_nous_auth_status_local() -> Dict[str, Any]:
    """Return a refresh-free Nous auth snapshot for read-only surfaces."""
    try:
        state = get_provider_auth_state("nous")
    except Exception:
        state = None
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    try:
        from hermes_runtime.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
...
def _confirm_expensive_model_selection(
    ...
    *,
    ...
    try:
        from agent.model_cost_guard import expensive_model_warning
...
def _prompt_model_selection(
    ...
    """
    from agent.model_catalog import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from hermes_runtime.config import save_config, load_config
    ...
    try:
        ...
        try:
            ...
            if model_ids:
                ...
                if free_tier:
                    try:
                        from agent.nous_account import (
