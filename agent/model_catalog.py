...
def _codex_curated_models() -> list[str]:
    ...
    """
    from hermes_cli.codex_models import DEFAULT_CODEX_MODELS, add_forward_compat_models
    return add_forward_compat_models(list(DEFAULT_CODEX_MODELS))
...
def static_provider_model_catalog() -> dict[str, list[str]]:
    """Return an isolated snapshot of the built-in provider model catalog."""

    return {provider: list(models) for provider, models in _PROVIDER_MODELS.items()}
...
def check_nous_free_tier(*, force_fresh: bool = False) -> bool:
    """Return false without consulting a Nous account or entitlement API."""
    del force_fresh
    return False
...
CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("nous",           "Nous API",                 "Nous inference API (direct API key)"),
    ...
    ProviderEntry("deepinfra",      "DeepInfra",                "DeepInfra (OpenAI-compatible direct model API)"),
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from agent.model_catalog import get_curated_openrouter_models
...
def get_curated_nous_model_ids() -> list[str]:
    """Return the curated direct-Nous model-id list.
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    """Return direct ``(api_key, base_url)`` credentials for Nous pricing.
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from agent.provider_auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    try:
        from agent.provider_auth import read_credential_pool
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from agent.provider_auth import fetch_nous_models, resolve_api_key_provider_credentials
            creds = resolve_api_key_provider_credentials("nous")
            if creds:
...
def should_use_copilot_responses_api(model_id: str) -> bool:
    """Public transport-selection predicate for GitHub Copilot models."""

    return _should_use_copilot_responses_api(model_id)
...
def fetch_deepinfra_models_by_tag(
    tag: str,
    *,
    timeout: float = 5.0,
    force_refresh: bool = False,
) -> Optional[list[dict]]:
    """Public tagged DeepInfra catalog contract used by provider plugins."""

    return _fetch_deepinfra_models_by_tag(
        tag,
        timeout=timeout,
        force_refresh=force_refresh,
    )
...
def validate_requested_model(
    ...
    *,
    ...
    if normalized == "moa":
        try:
            from hermes_runtime.config import load_config
            from agent.moa_config import normalize_moa_config
