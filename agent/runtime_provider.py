...
from agent import provider_auth as auth_mod
...
def _config_base_url_trustworthy_for_bare_custom(cfg_base_url: str, cfg_provider: str) -> bool:
    ...
    try:
        from agent.provider_auth import resolve_provider as _resolve_provider
...
def detect_api_mode_for_url(base_url: str) -> Optional[str]:
    """Public URL-based transport-mode detector for other application layers."""
...
def get_model_config() -> Dict[str, Any]:
    """Return the normalized active model configuration."""
...
def _copilot_runtime_api_mode(model_cfg: Dict[str, Any], api_key: str) -> str:
    ...
    try:
        from agent.model_catalog import copilot_model_api_mode
...
def _resolve_runtime_from_pool_entry(
    *,
    ...
    if not api_key and provider != "nous":
        api_key = getattr(entry, "access_token", "")
    ...
    if provider == "openai-codex":
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from agent.model_catalog import azure_foundry_model_api_mode
        ...
        if provider in {"opencode-zen", "opencode-go"}:
            ...
            # Refs #16878.
            from agent.model_catalog import opencode_model_api_mode
...
def get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    """Return a normalized configured custom provider, if one exists."""
...
def find_custom_provider_identity_by_model(model: str) -> Optional[str]:
    """Map a model id back to the configured custom provider that serves it."""
    ...
    providers = config.get("providers")
    if isinstance(providers, dict):
        for entry_name, entry in providers.items():
            if isinstance(entry, dict) and _entry_serves_model(entry):
                return f"custom:{_normalize_custom_provider_name(str(entry_name))}"
    ...
    for entry in custom_providers or []:
        if not isinstance(entry, dict):
            continue
        name = entry.get("name")
        if isinstance(name, str) and name.strip() and _entry_serves_model(entry):
            return f"custom:{_normalize_custom_provider_name(name)}"
    return None
...
def canonical_custom_identity(
    *,
    ...
    # 2. Recover by the session model when the endpoint was not persisted.
    if model:
...
def _resolve_explicit_runtime(
    *,
    ...
    if provider == "nous":
        direct_credentials = resolve_api_key_provider_credentials("nous")
        base_url = (
            explicit_base_url
            or _nous_inference_base_url_override()
            or str(direct_credentials.get("base_url") or "").strip()
            or auth_mod.DEFAULT_NOUS_INFERENCE_URL
        )
        api_key = (
            explicit_api_key
            or str(direct_credentials.get("api_key") or "").strip()
            or _getenv("NOUS_API_KEY", "").strip()
        )
        if not api_key:
            raise AuthError(
                "No Nous inference API key found. Set NOUS_API_KEY or add a "
                "Nous API-key credential with `hermes auth add nous`."
            )
        return {
            "provider": "nous",
...
def resolve_runtime_provider(
    *,
    ...
    from hermes_runtime.config import is_provider_enabled, load_config
    ...
    if pool and pool.has_credentials():
        ...
        pool_api_key = ""
        if entry is not None:
            pool_api_key = getattr(entry, "runtime_api_key", None) or ""
            if not pool_api_key and provider != "nous":
                pool_api_key = getattr(entry, "access_token", "")
        if (
            entry is not None
            and pool_api_key
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from agent.provider_auth import resolve_minimax_oauth_runtime_credentials
    ...
    if pconfig and pconfig.auth_type == "api_key":
        ...
        if not has_usable_secret(creds.get("api_key")):
            if requested_provider == "auto":
                logger.info(
                    "Auto-detected API-key provider %s without usable credentials; "
                    "falling through to the OpenRouter/custom endpoint resolver.",
                    provider,
                )
                runtime = _resolve_openrouter_runtime(
                    requested_provider=requested_provider,
