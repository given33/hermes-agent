...
class MiddlewareBackend:
    """Callbacks supplied by the plugin adapter at the composition boundary."""

    invoke: Callable[..., List[Any]]
    has: Callable[[str], bool]
    callbacks: Callable[[str], List[Callable]]


def _empty_invoke(_kind: str, **_kwargs: Any) -> List[Any]:
    return []


def _empty_has(_kind: str) -> bool:
    return False


def _empty_callbacks(_kind: str) -> List[Callable]:
    return []


_EMPTY_BACKEND = MiddlewareBackend(
    invoke=_empty_invoke,
    has=_empty_has,
    callbacks=_empty_callbacks,
)
_backend = _EMPTY_BACKEND
_backend_lock = threading.RLock()


def install_middleware_backend(
    *,
    invoke: Callable[..., List[Any]],
    has: Callable[[str], bool],
    callbacks: Callable[[str], List[Callable]],
) -> MiddlewareBackend:
    """Install the adapter used to access registered middleware.

    The services layer owns the execution contract but does not discover or
    import plugins. Composition roots install the plugin adapter explicitly,
    keeping the dependency direction from CLI/plugin infrastructure toward
    this framework-neutral service.
    """

    if not all(callable(item) for item in (invoke, has, callbacks)):
        raise TypeError("middleware backend members must be callable")
    global _backend
    replacement = MiddlewareBackend(invoke=invoke, has=has, callbacks=callbacks)
    with _backend_lock:
        previous = _backend
...
def restore_middleware_backend(backend: MiddlewareBackend) -> None:
    """Restore a prior backend, primarily for isolated application tests."""

    if not isinstance(backend, MiddlewareBackend):
        raise TypeError("backend must be a MiddlewareBackend")
    global _backend
    with _backend_lock:
        _backend = backend


def _current_backend() -> MiddlewareBackend:
    with _backend_lock:
        return _backend
...
def _invoke_middleware(kind: str, **kwargs: Any) -> List[Any]:
    return _current_backend().invoke(kind, **middleware_payload(**kwargs))


def _has_middleware(kind: str) -> bool:
    return _current_backend().has(kind)


def _get_middleware_callbacks(kind: str) -> List[Callable]:
    return list(_current_backend().callbacks(kind))
