...
位置:`plugins/collaboration/dashboard/`(`manifest.json` + `plugin_api.py` + `dist/` 前端)。功能:多 Profile **群聊**("Group chat turns execute the existing Hermes CLI against named profiles")、**rooms**、**Kanban 工作流**(渲染委托给内置 Kanban dashboard API)、移动端写审批恢复环。托管协作采用一个服务器本地 `hermes-manager` 调度员和三个独立 worker lane：`dbb3-worker`→`dbb3-primary`、`pc-worker`→`pc-primary`、`hk-worker`→`hk-primary`；没有 supervisor/reviewer 模型回合，worker 进度优先走认证 WebSocket，持久队列用于重连恢复。
