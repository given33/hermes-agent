...
# The tools layer owns the mutable primary skill root used by ``skill_view``.
# Runtime code receives that value through this narrow registration point
# instead of importing the tool implementation back into the foundation.
...
def register_primary_skills_dir_resolver(resolver) -> None:
    ...
    ``tools.skills_tool`` calls this once after defining ``SKILLS_DIR``.  The
    callable is intentionally evaluated at use time so profile switches and
    test overrides remain visible without a runtime-to-tools import.
    """
    global _primary_skills_dir_resolver
    if not callable(resolver):
        raise TypeError("skill directory resolver must be callable")
    _primary_skills_dir_resolver = resolver
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        if os.getenv("HERMES_KANBAN_TASK") or os.getenv("HERMES_KANBAN_BOARD"):
            ...
            try:
                toolsets = _load_raw_config().get("toolsets", [])
                result = isinstance(toolsets, list) and "kanban" in toolsets
            except Exception:
                result = False
    elif env == "docker":
        try:
...
def clear_external_skills_cache() -> None:
    """Clear cached external skill directories after configuration changes."""
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    # Resolve through the tools layer's registered callable at CALL time so
    # normalization and ``skill_view`` enforce the same root without making
    # this foundation import the tool package.
    try:
        primary_root = Path(_primary_skills_dir_resolver())
    except Exception:
...
NAMESPACE_PATTERN = _NAMESPACE_RE
