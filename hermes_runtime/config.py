...
# Keep the established operational logger name while the import path migrates.
# Dashboards and log filters already key on ``hermes_cli.config``.
...
from hermes_runtime.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        # Keep the compatibility runtime schema aligned with the official
        # v0.20 terminal backend contract.
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    """
    try:
        from hermes_runtime.skill_utils import discover_all_skill_config_vars, SKILL_CONFIG_PREFIX
    except Exception:
        return []
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    Fail-closed on a corrupt on-disk document. ``read_raw_config()`` degrades
    a YAML parse error to ``{}``, which a caller cannot tell apart from "no
    config yet" — and ``save_config`` writes with ``merge_existing=False``,
    so persisting that ``{}`` plus one migrated section would replace the
    user's entire config.yaml with just that section.
    ``require_readable_config_before_write`` does not close this: it proves
    the bytes are *readable*, not that they *parse*. That distinction matters
    most here, because migrations are the one write path a user reaches
    without asking for it — ``hermes update``, first launch after a version
    bump, ``doctor --fix``, and profile creation all run this unattended
    against whatever is on disk. A config with one bad indent must abort the
    migration and say so, not be silently replaced. Every production caller
    already catches and reports (``main.py`` prints "Config format update
    failed", ``doctor`` surfaces it as a warning), so raising degrades to a
    message rather than a crash.

    The check shares ``config_write_lock`` with the write so a sibling
    process cannot corrupt the file in the gap between them.
    """
    with config_write_lock():
        # Raises on a present-but-unparseable or non-mapping document;
        # returns {} for a genuinely absent file (first write creates it).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from hermes_runtime.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from hermes_runtime.toolset_validation import validate_platform_toolsets
    ...
    if missing_skill_config and interactive and not quiet:
        ...
        if answer in {"y", "yes"}:
            ...
            config = read_raw_config()
            try:
                from hermes_runtime.skill_utils import SKILL_CONFIG_PREFIX
            except Exception:
...
def read_raw_config(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``config_path`` overrides the default ``get_config_path()`` target for
    callers that resolve the config location themselves (doctor's
    ``HERMES_HOME`` module attribute, gateway handlers using the import-time
    ``gateway.run._hermes_home`` snapshot) — same raw semantics, same cache,
    just a pinned path. The cache is already keyed per-path so mixed callers
    can't cross-contaminate.

    Deliberately ignores ``HERMES_IGNORE_USER_CONFIG``: that flag suppresses
    user config at *load* (merge) time only. Raw reads feed write paths
    (``save_config`` explicit-path detection, ``mutate_config``), which must
    always see the real on-disk document.
...
def read_raw_config_strict(config_path: Optional[Path] = None) -> Dict[str, Any]:
    """Fail-closed variant of :func:`read_raw_config` for read-modify-write.

    :func:`read_raw_config` degrades every failure (missing file, parse
    error, permission error) to ``{}`` — correct for read-only "peek at one
    key" callers, but catastrophic as the read half of a read→mutate→
    ``save_config(..., merge_existing=False)`` cycle: a corrupt-but-
    recoverable ``config.yaml`` reads as ``{}`` and the subsequent save
    replaces the whole file with just the mutation, destroying every other
    section.

    This variant takes the same fail-closed stance as :func:`mutate_config`:

    - missing file → ``{}`` (first write creates it),
    - YAML parse error → propagates (caller must abort the write),
    - non-mapping document → ``ValueError`` (refuse rather than coerce),
    - unreadable file (permission/IO) → propagates.

    Use it whenever the result feeds a full-document write and the writer
    must stay :func:`save_config` (default-stripping, managed-key handling);
    callers that don't need ``save_config`` semantics should prefer
    :func:`mutate_config`, which bundles the same read with the lock and an
    atomic write. Deliberately uncached and lock-free: callers hold
    :func:`config_write_lock` around the whole cycle, and a strict read must
    see the true on-disk document, not a cache entry.
    """
    if config_path is None:
        config_path = get_config_path()
    try:
    ...
    if not isinstance(loaded, dict):
        raise ValueError(
            f"{config_path} is not a YAML mapping; refusing read-modify-write. "
...
def atomic_config_write(config_path: Path, data: Any, **kwargs: Any) -> None:
    ...
    # Serialize against other processes' config writers. This only covers the
    # write itself — read-modify-write callers must hold config_write_lock()
    # (or use mutate_config()) around their whole read→mutate→write cycle,
    # otherwise a concurrent writer between their read and this write still
    # loses. Reentrant, so callers already inside the lock pay nothing.
    with config_write_lock(config_path):
        require_readable_config_before_write(config_path)
        atomic_yaml_write(config_path, data, **kwargs)


# ── Cross-process config.yaml write lock ────────────────────────────────────
# _CONFIG_LOCK (top of module) only serializes threads *within* one process.
# The deployed topology genuinely has multiple concurrent processes doing
# read-modify-write on the same config.yaml: the long-lived gateway persists
# /reasoning and /fast (gateway/slash_commands.py), the dashboard writes
# through save_config, and platform adapters write back state (Telegram DM
# topic thread ids, Yuanbao auto-sethome). atomic_config_write guarantees an
# atomic *replace* but not mutual exclusion, so two interleaved
# read→mutate→write cycles are last-write-wins: whichever writer loses the
# race silently drops the keys the other one had just set.
#
# config_write_lock() adds a cross-process advisory OS lock —
# msvcrt.locking on Windows, fcntl.flock on POSIX — held for the whole
# read-modify-write. The lock target is `<config>.lock` NEXT TO config.yaml,
# never config.yaml itself: atomic_replace swaps in a new inode on every
# write, so a lock on the config file would silently stop covering it.
# OS region/flock locks are released automatically when their process dies,
# so a crashed writer cannot leave a stale lock (the empty .lock file left
# behind is inert).
_CONFIG_FILE_LOCK_TIMEOUT_SECONDS = 10.0
_CONFIG_FILE_LOCK_POLL_SECONDS = 0.05
# Reentrancy depth, guarded by _CONFIG_LOCK (which config_write_lock holds
# for its whole duration). Needed because Windows msvcrt.locking refuses a
# second lock on the same region even from the same process (second fd), and
# nesting is normal: mutate_config → atomic_config_write, save_config inside
# an explicit config_write_lock() block, etc.
_CONFIG_FILE_LOCK_DEPTH = 0

# Outcomes of a _try_lock_fd attempt. A plain bool cannot express the one
# distinction the polling loop needs: "another process holds the lock right
# now" (transient — the holder releases in milliseconds, so polling is worth
# it) versus "this filesystem/platform cannot take OS locks at all" (some
# network mounts — NFS without lockd, certain SMB/FUSE setups — reject
# flock/byte-range locking outright). The latter can NEVER succeed, so
# polling it would burn the full timeout on EVERY config write while holding
# _CONFIG_LOCK — a 10s stall per write, and a 10s event-loop stall when the
# writer is a sync function called from an async handler.
_LOCK_ACQUIRED = "acquired"
_LOCK_CONTENDED = "contended"
_LOCK_UNSUPPORTED = "unsupported"

# Errnos that mean "lock is held by someone else" — i.e. worth polling.
# POSIX flock(LOCK_EX|LOCK_NB) raises EAGAIN (== EWOULDBLOCK on mainstream
# platforms) or, on some systems, EACCES for a held lock. Windows
# msvcrt.locking(LK_NBLCK) raises EACCES for a held region and EDEADLOCK for
# its blocking variant's retry exhaustion. Anything else (ENOTSUP /
# EOPNOTSUPP / EINVAL / ENOLCK / EBADF, ...) means the lock operation itself
# is rejected here and retrying cannot change that. Built with getattr
# because a few of these names are aliases or absent on some platforms.
_LOCK_CONTENTION_ERRNOS = frozenset(
    getattr(errno, _name)
    for _name in ("EAGAIN", "EWOULDBLOCK", "EACCES", "EDEADLK", "EDEADLOCK")
    if hasattr(errno, _name)
)

# Lock-file paths (str) whose filesystem rejected OS locking as unsupported.
# Once a path lands here the OS-lock layer is skipped PERMANENTLY for this
# process — the filesystem will not start supporting flock mid-run — and
# config_write_lock degrades to the in-process _CONFIG_LOCK alone (exactly
# the pre-lock behaviour, minus the 10s-per-write poll). Keyed per path
# rather than a single bool because different config_path arguments can live
# on different filesystems (e.g. user config on an NFS home, a test tmpdir
# on local disk). Guarded by _CONFIG_LOCK: every reader/writer is inside
# config_write_lock, which holds that lock for its whole duration.
_CONFIG_FILE_LOCK_UNSUPPORTED_PATHS: Set[str] = set()
# The degrade is worth exactly ONE process-wide warning: it fires on every
# subsequent write, and a warning per write would drown the log on the very
# filesystems where it triggers. Also guarded by _CONFIG_LOCK.
_CONFIG_FILE_LOCK_UNSUPPORTED_WARNED = False


def _try_lock_fd(lock_file) -> str:
    """Non-blocking exclusive OS-lock attempt on open file ``lock_file``.

    Returns ``_LOCK_ACQUIRED`` on success, ``_LOCK_CONTENDED`` when another
    process currently holds the lock (transient — keep polling), or
    ``_LOCK_UNSUPPORTED`` when this platform/filesystem cannot take the lock
    at all (no fcntl/msvcrt, network mounts that reject locking): retrying
    an unsupported lock can never succeed, so the caller must degrade
    immediately instead of polling out the timeout.
    """
    try:
        if os.name == "nt":
            import msvcrt

            # Lock byte 0 (region locks may extend past EOF, so an empty
            # lock file is fine). Position matters to msvcrt — pin it.
            lock_file.seek(0)
        ...
        # ENOTSUP/EOPNOTSUPP/EINVAL/ENOLCK/EBADF and friends: the filesystem
        # (or this fd) rejects locking as an operation, not because a peer
        # holds it — no amount of polling will make it start working.
        return _LOCK_UNSUPPORTED
    except (ValueError, ImportError, AttributeError):
        # No usable locking primitive at all: fcntl/msvcrt missing on this
        # platform, or the file object is closed/detached. Same permanence
        # as an unsupported filesystem — retrying cannot help.
        return _LOCK_UNSUPPORTED


def _unlock_fd(lock_file) -> None:
    """Release the OS lock taken by :func:`_try_lock_fd`. Never raises."""
    try:
        if os.name == "nt":
            import msvcrt
...
def _note_lock_unsupported(lock_path: Path) -> None:
    """Record that ``lock_path``'s filesystem cannot take OS locks.

    Called under ``_CONFIG_LOCK`` (config_write_lock holds it for its whole
    duration), so the bare set/flag mutations are thread-safe. From here on,
    every ``config_write_lock`` on this path skips the OS layer and relies on
    the in-process lock alone — returning immediately instead of burning the
    full contention timeout on an operation that can never succeed.
    """
    global _CONFIG_FILE_LOCK_UNSUPPORTED_WARNED
    _CONFIG_FILE_LOCK_UNSUPPORTED_PATHS.add(str(lock_path))
    if not _CONFIG_FILE_LOCK_UNSUPPORTED_WARNED:
        _CONFIG_FILE_LOCK_UNSUPPORTED_WARNED = True
        logger.warning(
            "config_write_lock: OS-level file locking is unavailable on the "
            "filesystem holding %s; falling back to in-process locking only "
            "for the rest of this run. Config writes from OTHER processes "
            "are no longer excluded (concurrent writers may lose updates).",
            lock_path,
        )
...
def config_write_lock(
    ...
    """Cross-process advisory lock for config.yaml read-modify-write.

    Hold this around the WHOLE read→mutate→write cycle (or use
    :func:`mutate_config`, which does it for you). ``save_config`` and
    ``atomic_config_write`` acquire it themselves, so single-call writers are
    serialized for free; only multi-step read-modify-write callers need it
    explicitly.

    Degrades, never deadlocks: if the lock cannot be acquired within
    ``timeout`` seconds a warning is logged and the body runs WITHOUT
    cross-process exclusion, i.e. exactly the historical pre-lock behaviour.
    The failure mode under a stuck peer is therefore the old lost-update
    race, not a hang. Contention (a live peer holds the lock) is the ONLY
    thing that polls: a platform/filesystem that rejects locking outright is
    detected on the first attempt, degrades immediately (no timeout burned),
    and is remembered for the rest of the process — see
    :func:`_note_lock_unsupported`. Without that distinction every write on
    a lock-less filesystem would stall ``timeout`` seconds holding
    ``_CONFIG_LOCK``, which the sync-in-async call sites turn into a 10s
    gateway event-loop freeze per write.

    Also holds the in-process ``_CONFIG_LOCK`` for the duration, so a locked
    read-modify-write is atomic against sibling threads too. That means other
    threads' config reads can stall for up to ``timeout`` while a contended
    acquire polls — acceptable because writers hold the lock for milliseconds
    and contention implies a concurrent writer we must not race anyway.
    Reentrant within a process (nested blocks skip the OS lock). Keep the
    guarded body fast and free of blocking I/O.
    """
    global _CONFIG_FILE_LOCK_DEPTH
    with _CONFIG_LOCK:
        if _CONFIG_FILE_LOCK_DEPTH > 0:
            _CONFIG_FILE_LOCK_DEPTH += 1
            try:
                yield
            finally:
                _CONFIG_FILE_LOCK_DEPTH -= 1
        ...
        try:
            ...
            if str(lock_path) in _CONFIG_FILE_LOCK_UNSUPPORTED_PATHS:
                # This filesystem already proved it cannot take OS locks
                # (see _note_lock_unsupported). Don't even open the lock
                # file: the in-process _CONFIG_LOCK we already hold is all
                # the exclusion available here, and it is held either way.
                lock_file = None
            else:
                try:
                    lock_path.parent.mkdir(parents=True, exist_ok=True)
                    # "a+b" creates without truncating and never touches
                    # config.yaml itself; content of the lock file is irrelevant.
            ...
            if lock_file is not None:
                ...
                while True:
                    outcome = _try_lock_fd(lock_file)
                    if outcome == _LOCK_ACQUIRED:
                        acquired = True
                        break
                    if outcome == _LOCK_UNSUPPORTED:
                        # Locking can never work here — degrade NOW rather
                        # than polling out the timeout, and remember it so
                        # later writes skip the OS layer entirely.
                        _note_lock_unsupported(lock_path)
                        break
                    if time.monotonic() >= deadline:
                        logger.warning(
                            "config_write_lock: could not lock %s within %.1fs; "
                            "proceeding WITHOUT cross-process exclusion "
                            "(concurrent config.yaml writers may lose updates)",
                            lock_path,
                            timeout,
                        )
                        break
            ...
            # Fail open: the lock is an upgrade, not a gate. Any unexpected
            # error degrades to the historical unlocked behaviour.
            acquired = False
...
def mutate_config(
    ...
    **write_kwargs: Any,
    ...
    """Cross-process-safe read→mutate→write of the raw config.yaml.

    The single sanctioned pattern for "read config.yaml, change one thing,
    write it back". Under :func:`config_write_lock` it:

      1. re-reads the CURRENT on-disk raw YAML (never a cached copy — a
         sibling process may have written since the caller last looked),
      2. calls ``mutate_fn(config)`` with that dict,
      3. atomically persists the result via :func:`atomic_config_write`.

    ``mutate_fn`` may mutate in place and return the same dict, return a new
    dict to persist, or return ``None`` to skip the write entirely ("nothing
    changed"). It operates on the RAW on-disk document — no defaults merged,
    no env expansion — because persisting a defaults-expanded dict would
    freeze current schema defaults into the user's file. Keep it fast and
    free of I/O: the cross-process lock is held while it runs.

    Fail-closed on corrupt input: YAML parse errors propagate and a
    non-mapping document raises instead of being coerced to ``{}`` — a
    read-modify-write against an unparseable file must refuse rather than
    replace the user's config with just the mutation (same stance as
    ``atomic_config_write``'s readable-file guard). A missing file starts
    from ``{}`` (first write creates it).

    ``HERMES_IGNORE_USER_CONFIG`` deliberately does NOT apply here (nor to
    ``read_raw_config``): the flag suppresses user config at *load* time
    only; writers must always see the real on-disk document, otherwise a
    write under the flag would clobber the user's file.

    ``write_kwargs`` are forwarded to ``atomic_config_write`` (``sort_keys``,
    ``default_flow_style``, ...). Returns the dict that was written, or
    ``None`` when ``mutate_fn`` skipped the write.
    """
    path = config_path if config_path is not None else get_config_path()
    with config_write_lock(path, timeout=timeout):
        try:
        ...
        if loaded is not None and not isinstance(loaded, dict):
            raise ValueError(
                f"{path} is not a YAML mapping; refusing read-modify-write. "
                "Fix or remove the file first."
            )
        current: Dict[str, Any] = loaded if isinstance(loaded, dict) else {}
        result = mutate_fn(current)
        if result is None:
            return None
        if not isinstance(result, dict):
            raise TypeError(
                "mutate_config: mutate_fn must return a dict to persist or "
                f"None to skip the write, got {type(result).__name__}"
            )
        atomic_config_write(path, result, **write_kwargs)
        return result
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # honoured HERE, in the shared loader, with the same literal-"1" gate
        # as the legacy cli.load_cli_config(). Historically only the legacy
        # loader honoured it, so shared-loader callers (tools, gateway,
        # dashboard) kept seeing user config under the flag and
        # tools/delegate_tool.py grew a "prefer the legacy loader" workaround.
        # Semantics: the user file is treated as absent (defaults only), the
        # managed overlay still wins below (administrator pins are not a
        # user-side debugging surface), and .env credential loading is
        # unaffected. Load-path only by design — read_raw_config()/
        # save_config()/mutate_config() still see the real on-disk file, so a
        # save under the flag cannot clobber the user's config with defaults.
        ...
        if ignore_user_config:
            # Treat the user file as absent. The cache and last-known-good
            # stores are also bypassed further down: a defaults-only result
            # must never be served after the flag is cleared (the flag can
            # toggle within one process — tests, `hermes chat` subcommand
            # wiring) nor poison the corrupt-file fallback.
            user_sig = None
        ...
        if ignore_user_config:
            # Never cache or record last-known-good under the flag (see the
            # flag block above). `expanded` is a fresh unaliased dict, so it
            # is safe to hand out on both the deepcopy and readonly paths.
            return expanded
...
def save_config(
    ...
    *,
    ...
    Holds :func:`config_write_lock` (which subsumes the in-process
    ``_CONFIG_LOCK``) for the whole read→normalize→write cycle, so a
    concurrent gateway/adapter ``mutate_config`` in another process cannot
    interleave and drop keys this save is about to write (or vice versa).
    """
    with config_write_lock():
        if is_managed():
...
def save_env_value_secure(key: str, value: str, *, save_fn=None) -> Dict[str, Any]:
    """Persist an env value through an optionally injected lifecycle writer.

    The runtime foundation owns the physical ``.env`` write.  Application
    surfaces inject the provider credential lifecycle so credential-pool and
    config mirrors are reconciled without making this module import the CLI.
    """
    writer = save_fn or save_env_value
    writer(key, value)
    return {
...
def redact_config_value(value: Any, _depth: int = 0) -> Any:
    ...
    :func:`hermes_runtime.redaction.mask_secret`. Non-secret keys and scalar values pass
...
def show_config():
    ...
    anthropic_value = next(
        (
            ...
            if (value := get_env_value_prefer_dotenv(env_name))
        ),
        "",
    )
    ...
    # Skill config
    try:
        from hermes_runtime.skill_utils import discover_all_skill_config_vars, resolve_skill_config_values
...
def set_config_value(
    ...
    *,
    ...
    if _is_env_config_key(key):
        writer = credential_saver or save_env_value
        writer(key.upper(), value)
...
def unset_config_value(key: str, *, credential_remover=None):
    ...
    if _is_env_config_key(key):
        if credential_remover is None:
            found = remove_env_value(key.upper())
        else:
            found = bool(credential_remover(key.upper()).get("found"))
        if not found:
            print(f"Config key not set: {key}", file=sys.stderr)
            sys.exit(1)
...
def config_command(args, *, credential_saver=None, credential_remover=None):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            ...
            sys.exit(1)
        unset_config_value(key, credential_remover=credential_remover)
...
# Public configuration primitives used by other runtime layers.  The
# underscore-prefixed names remain available for historical integrations, but
# production callers should use these stable names so package boundaries do
# not depend on implementation details.
deep_merge = _deep_merge
expand_env_vars = _expand_env_vars
normalize_root_model_keys = _normalize_root_model_keys
sanitize_env_lines = _sanitize_env_lines
