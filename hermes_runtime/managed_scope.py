...
This is DISTINCT from ``hermes_runtime.config.is_managed()`` / ``HERMES_MANAGED``,
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    (rather than going through hermes_runtime.config.load_config) to honor managed
    scope. Mirrors hermes_runtime.config._load_config_impl's managed merge exactly:
    ...
    try:
        ...
        from hermes_runtime.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
