...
def _write_env(env_path: Path, env_writes: dict[str, str]) -> None:
    ...
    # env_writes carries MEM0_API_KEY / provider API keys — use the
    # TOCTOU-safe secret writer (temp file pinned to 0o600 before any
    # secret byte is written) instead of write_text, which creates the
    # file with umask-default (typically world-readable) permissions.
