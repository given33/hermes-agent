...
def _open_session_db_at_path(db_path: Path, *, read_only: bool):
    ...
    try:
        ...
        if not stale_schema and not (
            # UnicodeDecodeError means pysqlite could not decode SQLite's own
            # error message because corrupt file bytes were embedded in it
            # (#98924). Route it through the same one-writable-open repair
            # path as malformed schema errors.
...
async def list_credential_pool(profile: Optional[str] = None):
    def _run() -> Dict[str, Any]:
        ...
        with _config_profile_scope(profile):
            providers = []
            # read_credential_pool(None) lists every provider that has pooled
            # entries; load_pool() gives us rich objects in the same profile.
        ...
        with _config_profile_scope(profile):
            ...
            if step is not None:
                try:
                    ...
                    # Cleanup is best-effort, but suppression is the sticky
                    # boundary that prevents the source from reappearing.
                    _log.exception(
