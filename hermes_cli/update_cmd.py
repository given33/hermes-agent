...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    ...
    try:
        ...
        # urlretrieve takes no timeout: a stalled CDN connection hung
        # `hermes update` forever with no hint. Bound it with the process
        # default socket timeout (urlretrieve's urlopen inherits it) and
        # restore it afterwards; failures flow into the surrounding except.
        import socket as _socket

        _previous_default_timeout = _socket.getdefaulttimeout()
        _socket.setdefaulttimeout(60)
        try:
            urlretrieve(zip_url, zip_path)
        finally:
            _socket.setdefaulttimeout(_previous_default_timeout)
