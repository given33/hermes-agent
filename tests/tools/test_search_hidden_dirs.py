...
_HAS_POSIX_FIND = os.name != "nt" and shutil.which("find") is not None
_HAS_POSIX_GREP = os.name != "nt" and shutil.which("grep") is not None
...
@pytest.mark.skipif(not _HAS_POSIX_FIND, reason="POSIX find is not available")
...
@pytest.mark.skipif(not _HAS_POSIX_GREP, reason="POSIX grep is not available")
