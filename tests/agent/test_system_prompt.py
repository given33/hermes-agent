...
def test_coding_prompt_preserves_legacy_workspace_order(monkeypatch):
    ...
    # Production renders prompt paths with forward slashes on every host.
    # Normalize the platform-native Path representation before comparing.
    _home_str = str(Path("/hermes")).replace("\\", "/")
    expected_profile = (
...
class TestSessionStartLike:
    ...
    def test_nonmatching_session_id_uses_session_start(self):
        ...
        agent = SimpleNamespace(
            ...
            # Use a midday local stamp so converting to UTC cannot cross into
            # the previous calendar day on a UTC+/- offset CI worker.
            session_start=datetime(2026, 1, 1, 12, 30),
        )
        start = _session_start_like(agent, now)
        assert start.strftime("%Y-%m-%d") == "2026-01-01"
