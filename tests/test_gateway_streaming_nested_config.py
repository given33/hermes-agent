...
def _load_with_yaml_dict(yaml_dict: dict):
    ...
    with patch("gateway.config.get_hermes_home", return_value=fake_home), \
         patch.object(Path, "exists", lambda self: str(self).endswith("config.yaml")), \
         patch("hermes_runtime.config.read_raw_config", return_value=yaml_dict), \
         patch("hermes_runtime.managed_scope.apply_managed_overlay", side_effect=lambda value: value):
